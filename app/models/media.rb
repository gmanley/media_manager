...
class Media
  ...
  after_create :async_generate_thumbnail
  ...
  def generate_thumbnail
    ...
    Dir.mktmpdir do |dir|
      thumb_path = File.join(dir, "#{File.basename(file_path)}_thumb.png")
      Rails.logger.warn(thumb_path)
      if system("ffmpeg -ss #{duration / 2} -i #{file_path.shellescape} -vcodec mjpeg -vframes 1 -an -f rawvideo #{thumb_path.shellescape} >/dev/null 2>&1")
        self.poster = File.open(thumb_path)
      else
